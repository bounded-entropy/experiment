...
93. **A ROUTED BANK ENTRY: ONE ADAPTER TYPE HOLDS A DREAMER AND N MEMORIES; ROWS ROUTE BY THE PLAN LEAF'S ROLE, REQUESTS BY A DIRECTIVE (ADR 0018, 2026-09-17).** Samarth's ask (research-log post 0007): a population of N memory LoRAs learning a text stream on the spot, k free-form dreams per arrival each trained into m memories, a prequential contrast on the second half of a split passage as the dreams' reward, GRPO on the dreamer, an anchored quadratic penalty from a calibrated input covariance — through the desk on Strange Loop, every primitive change recorded before it is made. Implemented on `claude/dreams` off `claude/adr-0014-commit-cadence`; fakes suite 1520 OK, 205 skipped; the torch-gated bank and loss tests under the factual-env interpreter; four smoke shapes on a 0.5B through the desk; the 7B calibrate, base-dreams teacher and four stream arms on metal.
    - **A — the entry.** `dream_bank` (`policy/adapters/dream_bank.py`, `_torch.py`, `_vllm.py`): one bank entry holding N+1 LoRA sets (`dreamer`, `memory:00`…), one payload (`dream_bank/1`: every set's peft fragment plus the anchor's factors), one bundle id, N+1 punica slots priced by `max_members`. The torch site routes each row to its set from the row's turn facts; a uniform microbatch takes the single-tenant GEMM pair, a mixed one takes ONE pair per distinct route assembled with `index_add` (the per-row gather made a 7B step 3× slower per token). The vLLM lowering writes one peft dir and one `LoRARequest` per route and applies the `Route` directive's set per request, recording `route` in the turn facts. Calibrate mode accumulates Σxxᵀ per site; `factorize` keeps the top `anchor_rank` eigenpairs and a diagonal remainder; `provide` yields `anchor_penalty` (λ·Σ_memories ½tr(ΔW C ΔWᵀ)), `anchor_raw` (unscaled, so a λ=0 arm reports the magnitude), and the two delta norms.
    - **B — roles are routes.** `data/plan.py`: a leaf role that is neither `train` nor `eval` names the set the row trains under; `realize` stamps `role` and `route` into a deep copy's turn facts (a `train` row is returned untouched, byte for byte). The Scorer pins a wave whose sampled turns disagree, or that samples nothing, to the ledger's bundle at u−1 (initial at u=1), waiting for that commit; only pins whose version map matches this run's bank vote. The `dream_stream` loss reads roles off `doc_turn_extras`: memory rows clone, dreamer rows take the PPO-clip surrogate over the `advantage` column, eval rows mask, and the penalty is added once per update.
    - **C — evaluation is rollout traffic; an eval row is never forwarded.** The `answer` environment (greedy, 16 tokens, stop at the line, sampling of its own — `Environment.sampling` overrides the run's) asks each question under a `Route`; `gold_logprob`, `squad_answers` (official EM/F1), `squad_judge` (SEAL's yes/no under a judge pool) grade it. The Trainer's `trained_rows` drops role `eval` before flatten: the learner pads a microbatch to its longest row, and 54 short answers beside long dreams asked for 8.5 GiB of logits.
    - **D — the batch.** A desk seals every update at a fixed cost, so one update trains `arrivals_per_update` arrivals (8): one TEXT group per arrival (its text under every memory, and at a second half the first half's dreams under the dreamer), one DREAM group per arrival after all text groups (the packer keeps like lengths together), the answers last; a second half in its first half's batch is refused; the contrast's nuisance counts every other dream trained from the first half's batch through the batch before the second half's.
    - **E — the venue, seven fixes.** Scratch client: request deadlines grow with the bytes both ways (a 14 MB plan, a 12 MB task set), and a 5xx on a READ is retried (a 500 on a ledger read ended a run and its lease three times); provider: 300 s for a fresh pod's SSH metadata, a lease reported `failed` is ENDED (it never becomes `released`), a lease the desk cannot reconnect at startup is reported, not fatal; desk: a solo submission's placement mode rides the archive and a reroute replays it; the deploy door gained `--solo` and `--delivery`. The wire is relayed through the desk's machine: a 17-set 7B bank with its rank-128 anchor is ~260 MB per update (184 s per seal), so a stream run's engine and learner ALTERNATE on one host with the engine's sleep mode on (seal 13 s); store delivery would checkpoint ~1 GB per update at the pod's ~5 MB/s.
    - **F — the afternoon's fixes, each after a death on metal.** Scratch client: a read owns five tries with the full timeout each (one 30 s stall ended a host); desk: the startup loop reconnects saved leases BEFORE the runtime ticks (a restart under recovery reaped two live hosts), a solo submission's placement mode rides the archive and a reroute replays it, `automatic_recovery` on for this desk (under `false` a dead listing blocks every placement and no operator verb drops it); `pack`: the microbatch budget is the PADDED footprint, rows × longest (24 dream rows beside one 712-token dream asked for 10.7 GiB of logits); `dream_effect` emits `prequential_base` (the untouched base's surprise at each text, once per text) and every eval group asks the `base` route too — drift and learning read against the same model at the same time. The first stage-A arms at memory lr 2e-4 (8 steps per batch) collapsed within forty passages (surprise at new text 1.16 → 2.55, EM 0.25 → 0.04, λ=2e-3 holding nothing); the campaign runs at lr 5e-5 with λ_high 2e-2.
    - **G — the store's write outcomes, and what the lr 5e-5 arms showed.** ADR 0015's stop is kept for a write with no answer (it may still land: never replayed), but the API ANSWERING a write with a server error has refused it — `ScratchRejected`, tried `WRITE_ATTEMPTS` times (the desk's owner had stopped four times on fleet-journal appends answered 500); a lost acknowledgement is followed by the readback, and the offered bytes coming back mean the publication stands. `DreamSettings.dream_prompt` names the dream instruction (`useful`, or SEAL's `implications`); the task id says which. At memory lr 5e-5 over eight batches the memories neither drifted (surprise within ±0.15 of the base's at every text) nor learned (EM equal to the `base` route's on every batch), and restatement dreams moved the second half's surprise by −0.002 ± 0.003 over 60 dreams; the campaign's controls are now SEAL's — implications replayed into memories at lr 2e-4 beside a text-only arm and a λ=2e-2 arm, all read against the base.
    - **H — the token rotates, the API goes away, and a run waits.** Twenty minutes of the scratch API answering 500 and timing out (14:05 EDT) ended every 7B arm at its next store call, the idle rule decarved the emptied hosts, and the desk's owner stopped on a fleet-journal append whose readback was refused: the browser session's token had expired ten hours in, and every pod held a copy baked into its worker environment at boot, with no way to receive another. Scratch client (`_open`): a 5xx or a dropped connection is judged by one question — does `/auth/me` answer? — and an API that is DOWN is waited out (pauses doubling to 60 s, for `OUTAGE_CEILING_S` = 8 h, noted every ten minutes), a run paused in a store call resuming the moment the API does; a 401 re-resolves the credentials from where they came (the profile a login rewrites, the token file a reauth rewrites), a renewed token retrying at once and an unchanged one waiting the same way, since only a person can renew it; an API that is UP keeps F's rules (five tries for a read, one for a write), and a REFUSED write has nothing in flight and waits like a read. Pods resolve credentials through `SL_API_TOKEN_FILE` (`/tmp/rlstack-<lease>-credentials.json`, written at boot) instead of a token in their environment; `reauth` — a venue-door verb beside `boot`/`release`/`export`, `deploy/strangeloop.py reauth [metal]`, and what `serve` runs for every booted lease it reconnects — rewrites that file from the profile. Every arm is resubmitted with a checkpoint every 8 updates (one batch: the 64-update cadence saved nothing when the arms died at updates 8–40). The config declares three metals at `devices: 4` and 23-hour leases (Modal's A100-80GB nodes come in 1/2/4/8 GPUs — a `--count 3` is rejected before provisioning — and `DeskConfig` refuses slots whose devices sum past `gpu_ceiling`): the provider already passes `--count`, the GPU ceiling counts GPUs, and the desk carves per device (a solo arm takes an empty metal first, then a free device on an occupied one), so twelve GPUs sit on three of the account's four leases. 1535 tests, 205 skipped.
    - **I — the desk hosted: the laptop's standing process in one Modal container.** The Strange Loop desk was a laptop process, so the laptop had to stay on for every lease's tunnel; `deploy/strangeloop_desk.py` (app `rlstack-strangeloop-desk`, yu-masala workspace, `min_containers=1`/`max_containers=1`, 24 h timeout, the same keep-alive as `deploy/desk.py`) runs the SAME `serve(config, source_root)` as a task on the container's loop from `@modal.enter` to `@modal.exit`, and nothing about placement, leases or addresses moves: the container opens the outbound ssh per lease, the pod reaches the gateway at its own loopback through that ssh's reverse route, every recorded address stays `http://127.0.0.1:<gateway_port>#…`. The container carries what the laptop carried — `openssh-client`, the CLI as the published 0.8.1 wheel (`cli.strangeloopresearch.com`; `strangeloop-cli` is NOT on PyPI, 404 both endpoints), `SL_API_TOKEN` from the secret `strangeloop-token` (the CLI's `config.resolve` reads it on every command, so nothing logs in), the two package trees whole under `/root` (`source_bundle` ships the observer's assets too, so `add_local_dir`, not `add_local_python_source`), and one volume `rlstack-strangeloop-desk-state` at `/state`: `/state/desk` the desk's state dir, `/state/strangeloop` the CLI's config dir (`STRANGELOOP_CONFIG_DIR`; its `ssh/id_strangeloop` is what every pod it boots trusts, and `gpu forward` rewrites `ssh_config` per lease, so only the key needs seeding). Four small primitives made it possible, each a rule with a test (`tests/test_strangeloop_hosted.py`, 16): `DeskConfig.read(path, state_dir=)` puts the volume in the file's field's place; `DeskConfig.operator_endpoint` (a bare http(s) origin, `check_operator_endpoint`) with `operator_door` — the operator's verbs in `deploy/strangeloop.py` knock there, the workers' `endpoint` never moves; `operator_token` takes `RLSTACK_HTTP_TOKEN` from the environment first (copied once: `modal volume get … desk/http-token -`) and REFUSES for a hosted desk without it rather than mint a local token the desk would 401; `serve(…, exposed=True, stop=)` binds `0.0.0.0` (Modal's ingress refuses a loopback bind; `HttpServer` already demanded a token off loopback) and takes a stop event from the host instead of signal handlers, and the observer off loopback is wrapped by `observer_guard` — the bearer, or `?token=` once which sets an HttpOnly cookie, else 401. The lock: `own_local_desk` writes a `LockHolder` (host, pid) into `desk.lock`; flock fences where the filesystem fences and a refused flock (a mounted volume) falls back to the record — a record another machine wrote or whose pid is gone is stale and taken, a live desk's on this host is refused — so a container that died never bricks the next start. `examples/strangeloop-dreams-hosted.json` is the dreams config plus the door; `reauth` with no slot named asks every slot of a hosted desk (`renewable`), since the booted files are on the volume. Migration is documented in `examples/strangeloop-desk.md` ("The desk hosted"): stop the laptop desk (one owner per journal, ADR 0015), seed the volume with `~/.config/strangeloop/ssh` and `~/.local/state/rlstack-strangeloop-dreams`, deploy, and the hosted desk reconnects the laptop's leases with the laptop's key. NOT PROVEN — nothing was deployed: the wheel install in the image, `@modal.web_server` on two class methods (Modal 1.5.4 admits it on methods, `require_sync`/`require_nullary`), the 900 s `startup_timeout` against a startup that reconnects every lease before binding, flock and directory-fsync on a Modal volume (`save` fsyncs the state dir), the volume's background commits landing the lease files, and the workspace slug in the example's URL (`modal deploy` prints the real one). Fakes suite 1551 OK, 205 skipped.
    - **I, run on Modal (2026-09-17, 18:51 EDT).** `modal deploy deploy/strangeloop_desk.py` stood the app up once the image's one mount-only step came last (Modal refuses a `copy=True` `add_local_*` after a mount; 5e81075): the 0.8.1 wheel installs from `cli.strangeloopresearch.com` (a 307 to `/cli/…`, 137 kB, `Successfully installed strangeloop-cli-0.8.1`), both `@modal.web_server` methods deploy, and the printed URLs are `https://yu-masala-workspace--rlstack-sl-desk-gateway.modal.run` and `https://yu-masala-workspace--rlstack-sl-desk-observer.modal.run` — the example's `operator_endpoint` as written. On the volume (probed from a throwaway container): flock is GRANTED, `save()`'s file and directory fsyncs and the rename work, and the desk's `desk.lock` record landed as `modal 2` — every Modal container is host `modal` with the desk at pid 2, so a host+pid record could not tell a died container from the live one; `LockHolder` now carries the kernel boot id (fresh per container, empty on the laptop) and a record under another boot is stale (469036d, one test). Two things stopped the first two containers before they bound. (1) The secret `strangeloop-token` held the doc's literal placeholder `slk_...` (7 bytes; the API answers "Invalid or revoked API token"), so `serve` sat in the scratch client's 401 wait (`scratch GET waiting: credentials refused (HTTP 401) … retrying for up to 28800 s`) inside `own_local_desk`; the secret was rewritten from the CLI profile (`--from-json`, the value never shown) and the migration command now reads it from there (18eb58f). (2) With a valid token the container stayed silent: py-spy inside it showed Modal's main thread in `wait_for_web_server` and no frame of `serve` anywhere — Modal drives its user event loop only while a hook or an input runs, so the task `bring_up` created got one step (lock, token, store client, the first CLI `status` on an executor thread) and froze at its first await; when the container was later terminated, the exit hook drove the loop again and the frozen desk reconnected, bound and stopped within a second, which is the proof. `DeskThread` (3e2ce5f) runs `serve` under `asyncio.run` on a daemon thread of its own and the exit hook stops it through that loop; the third container bound `desk: http://0.0.0.0:18780; observer: http://0.0.0.0:8341` thirteen seconds after the deploy, and `status`/`dispositions` through the gateway with `RLSTACK_HTTP_TOKEN` answer: three metals a/b/c, all `released: true`, no listings. STILL UNPROVEN: the reconnect loop against a live lease (the seeded files name released leases, skipped silently), a lease file the desk writes landing on the volume through its background commits, a container replaced under live tunnels, the observer through a browser. The laptop poller (`~/.local/state/rlstack-strangeloop-dreams/podlogs/poll.sh`) discovers leases from `strangeloop gpu list --json` by label now, the local state files being stale.
    - **J — the partition, the group difference, plain REINFORCE (Samarth, 2026-09-17 evening).** The random draw of `memories_per_dream` memories per dream (2 of 16: half a dream per memory per arrival, one eighth of SEAL's dose, and the reason the implications control read null in aggregate while its per-memory split moved) is replaced by THE PARTITION: every arrival shuffles the memories and cuts them into one equal group per dream (`assign_dreams`; `memories_per_dream` is now derived, `memories % dreams_per_arrival == 0` enforced), so each memory trains on exactly one of the arrival's dreams. The reward is the GROUP DIFFERENCE — the mean second-half NLL of the memories outside a dream's group minus the mean inside it (`dream_effect.contrast`; positive is helpful) — and the windowed ridge with its nuisance count is gone (`contrast_of` carries only the first half's index and the partition). `dream_advantage` subtracts the arrival's mean dream reward (no z-score); `dream_stream` is REINFORCE — the advantage times the dream's token log-probabilities, no clipped ratio, since the dreamer takes one on-policy step per batch. The stream's gap and panel are `build_stream` parameters and `stream` flags (`--gap-min/--gap-max/--panel`), recorded in the stream file; an answer-length knob was tried and reverted (the environment declares its sampling; threading a per-task limit through the client was the plumbing the day was cutting). Decided with it: the anchored penalty stays available but every arm runs at λ=0 with plain LoRA memories, and exactly one implications arm remains as the SEAL-equivalent reference. 1552 tests, 205 skipped; the torch-gated dreams tests green under the factual-env interpreter. Every prepared arm is re-prepared under this shape (the plans changed).
    - **K — the judge is a standing pool the arms dial (Samarth, 2026-09-17 evening).** Exact match leaves the reads: SQuAD's gold answers are spans of a passage the memories no longer see, so string match grades phrasing, and the three-shot short-answer prompt was a hack around it. The grader is `squad_judge` (SEAL's yes/no question, greedy, on a `judge` pool serving Qwen2.5-32B-Instruct, `--judge-base`/`--judge-gb`), in the run, on the flagged rows only; the driver's analysis reads its `judged` column as `acc`/`base_acc` and the panel keeps only gold-span log-probability. Two rules make the judge ONE daemon rather than one per arm: `campaign.demands_of` stamps NO adapter type on a pool declared on another base (nothing of this bank can live there — a FROZEN pool), and `place_listings` lets a `frozen_unit` (inference demands naming no adapter type) keep the plain avoid set under `solo`, so the first arm carves the judge on a free device and every later solo arm JOINS it while carving its own main and learner (`tests/test_desk.py::test_a_solo_submit_s_frozen_pool_joins_the_standing_judge`). The judge's traffic (a few hundred four-token calls per batch) crosses the desk's relay; a local Claude-API grader was written and withdrawn the same evening. 1553 tests, 205 skipped.
    - **L — the first evening on the hosted desk, four primitives (2026-09-17, 19:20–22:30 EDT).** Five arms launched through the hosted desk on two 4-GPU leases with the judge on slot a's device 2; within the hour every arm was dead, each death a primitive's edge. (1) The three arms on slot a died to the resident watchdog: their learners were silent past 600 s inside `emit` while the 32B judge loaded 65 GB from a network volume beside them — `RESIDENT_STALL_S` is 1800 now; the bound guards a wedged pipe, not a slow node. (2) The desk's owner stopped on a fleet-journal append the API answered 500 three times while `/auth/me` answered: a refused write is never in flight, so `_open` now retries it to the ceiling whatever the health question says — and since the desk appends its journal from its event loop at eighteen sites, that wait FROZE the desk; the fleet journal (observability only, by base.py's own word) is now written off the loop by one ordered writer thread, drained by readers and persist, the failing write stopping the owner for the next append. (3) The two arms on slot b died at a desk redeploy: their Scorer's ask to the judge on slot a relays through the desk, the container was replaced mid-call, and `Unreachable` ended the adoption — `HttpTransport.ask` now retries a TRANSIENT unreachable (the transport's own, marked by `transient()`) for `ASK_RETRY_S` = 600 s, calls never; and a frozen unit now prefers its own arm's metal (`beside`: a join there, else a carve there, else the fleet's rungs), so each node carries its own judge and judge calls stay in-process (`test_a_frozen_pool_stays_on_its_own_arm_s_metal`). (4) The hosted door's ingress answers 303 after ~150 s, longer than a submit's carve-and-build; a submit from inside the container at the loopback door has no such limit (`insubmit.py`, an operator script for now). Found beside these: the scratch API refused every write with 503 from ~20:30 EDT for over an hour while reads answered; every store write in the fleet waited by design, which is why submits hung at the desk (they await the hosts' adoption writes). Tests 1560, 205 skipped.
    - **Unproven, stated:** the anchored penalty's effect (the λ arms are running); the trained dreamer's rewards beyond the smoke (±0.018 at the second halves on the 0.5B, the dreamer's delta norm moving); the judge pool on metal (no fifth lease); the lamp family (`data/tasks/lamp_dreams.py` deferred); what host RAM a 4-GPU node carries under four engines; a paused run actually resuming after a `reauth` on metal (the rule is tested against a fake API only). The account holds four leases; two fresh pods never published SSH.

94. **ADAPTERS GET NAMES; A FIT IS A CLIENT LOOP OVER THE LEARNER'S VERBS; TWO ADAPTERS STACK ON ONE REQUEST (ADR 0019, 2026-09-18/19).** Samarth's ask (research-log post 0008): hundreds of small SEAL-style LoRA fits, a dreamer sampled and trained STACKED on a fitted memory, runs that consume adapters other runs are still writing, all through the desk, built in parallel. His correction shaped it: the learner is already the engine's backprop twin, so there is no fit verb and no new resident — what was expensive was the Trainer role's seal, not the learner. Built as six parallel streams in their own worktrees against the ADR's signatures and merged on `claude/dreams`; fakes suite 1835 OK, 249 skipped; every torch-gated test passes under the factual-env interpreter.
    - **Names.** `Store.write_named / read_named / named_meta / promise_named / named_state` (keys `runs/<subdir>/names/<name>.bin|.json`; the meta seals the bytes; write-once). A writer promises EVERY name in one file (`names/_promises/<digest>.json`; the per-name files of the first night are still read): per-name promises cost ~10 s each on the scratch store, half an hour before a 200-job run's first step. `runner/names.py`: `lib_names`, `names_ready` (present → go; promised/unknown → wait with a note every 600 s; orphaned → `NamedAdapterOrphaned`). The Generator waits and hands engines bytes (`Engine.add_library`); the Trainer waits, `load_set`s library sets before the wave and `drop_set`s them after.
    - **Learner.** `load_set` (None re-initializes; moments reset; `lib:` installs a frozen set outside every optimizer), `drop_set`, `emit_set`, `forward` (no grad, per-document mean NLL over the loss mask), `optim_step(tenant, lr_scales)`. `dream_bank` has one param group per route, clips gradients PER GROUP (`AdapterType.clips_by_group`: a joint norm let one lane's gradient scale its neighbours' steps), and replays a two-part route as the sum of deltas with every `lib:` part detached. A NAMED PAYLOAD is exactly `lora_torch.emit` of one set (fp32, scaling 1).
    - **Engine.** `Engine.add_library` (LRU by `EngineBuild.max_library`); `lib:<name>` served alone, `lib:<name>+dreamer` as one concatenated LoRA materialized per (bundle, name), exact at scaling 1; a rank above `max_rank` refused at materialization with both numbers.
    - **Fit runs.** `Plans.fit` (a jsonl of `FitJob` rows; enters identity only when set), `runner/fit.py::run_fits` (lanes = routes of one tenant, per-lane linear decay through `lr_scales`, probes before training and after every stage), `roles/fitter.py` (promises at birth, the name before its ledger line, one ledger line per job in plan order, resume = skip present names), loss `sequence_sft`. `PostProcessor.fits` puts a processor inline with `client.fit` (`forks`, `tokenize`, `rows`) and `client.names`.
    - **Site grammar.** A segment may be `a|b|c` (`layers.*.self_attn.q_proj|v_proj`).
    - **What metal taught the first night.** Pods run the HF hub offline: a base missing from the shared cache failed the carve, and the desk said only "no registered metal can hold them" — `carve_on` now journals `carve-refused` with the reason. A recipe pays adapter slots inside EVERY engine on the metal: rank-32/64 slots put 13–26 GB inside the 32B judge and its KV cache went negative — a frozen unit is now carved with `serving_nothing(recipe)`. A whole-card demand (`vram_gb=None`) and an explicit one behaved the same; state it anyway. Store calls are 3–10 s each and serialized per host, so large fit runs are sharded one per host.
    - **Unproven, stated:** cross-tenant continuous batching on the learner (not built); stage two of the ADR (bank as a view, pull-by-name delivery, WarmStart on names); vLLM with per-module tensors narrower than the stack's config rank; a Fitter killed mid-job and resumed on metal.
...
90. **TRAINING COMMITS; INFERENCE LOADS (ADR 0011, 2026-09-13).** Completes
    ADR 0006's learner-only shape. The Trainer has no engine, route callback,
    or inference admission: it persists adapter/optimizer blobs and appends
    the ledger without installing a serving bundle. Injected text uses the
    learner's base tokenizer through `tokenize(tenant, text)` (wire name
    `learner_tokenize`), cached per Trainer with a 4,096-entry bound. Generated
    token IDs remain recorded data. Torch/FSDP use the same no-special-token
    convention as VllmEngine; the FSDP lead tokenizes without collectives.

    Generator and Scorer restore their selected version from storage, then
    install it into only the pools each daemon declares. No additional daemon.
    Version zero remains the explicit bootstrap; subsequent selection uses
    committed ledger pins or the scoring wave's recorded pin. Base-only pools
    also reload on eviction. Replay without pooled scoring needs no main pool;
    main scoring traffic must explicitly declare main. Pool-less processors
    construct no default route until they actually request traffic.

    Validation: 1,399 tests completed, 1,207 passed and 192 dependency/GPU skips.
    Unchanged online, replay, judge and policy-scoring specifications retain
    identical run IDs and every stored byte against baseline e5739ba. Covered
    delayed inference, partial writes, install failure/restart, historical pins,
    base eviction and remote learner-only execution. No live GPU or volume
    drill was run; tokenizer parity and FSDP on metal remain unverified. The
    existing TaskGroup still propagates fatal daemon failures across the run;
    this changes the storage boundary, not independent daemon supervision.

91. **PROVIDERS SHARE DESK AND METAL RUNTIMES (ADR 0017, 2026-09-13) — THE STRANGE LOOP BACKEND PUBLISHED (ADR 0014, PART E, STEP 1).** Samarth requested real abstractions after the initial Strange Loop file relocation; the work lived only as working-tree state in the `codex/strangeloop-backend` worktree until this entry, and lands here as a reviewed series with its ADRs renumbered (0011→0015 the scratch store, 0012→0016 the HTTP blobs, 0013→0017 the shared runtimes; 0011 on main is `training-commits-inference-loads`, #90). Both `runner/venues/modal/` and `runner/venues/strangeloop/` have provider, desk and worker adapters and instantiate the same DeskRuntime and MetalRuntime; AllocationProvider supplies boot and exact-allocation termination; VenueClient owns canonical submission, readiness polling, guarded release and observer progress. Deployment files keep resources, operator commands and compatibility wrappers.

    Modal's idle/recovery clocks now start from async enter and live in DeskRuntime; the separate Modal reaper schedule is removed. Worker registration, heartbeat, host supervision and teardown are shared. Released runtimes refuse new work. Existing Store commit points own publication. The fourteen canonical venue-spec fixtures stay unchanged; new adapter tests cover both startup paths and the shared lifecycle. Architecture tests forbid provider dependencies in shared runtime/client modules and imports between providers. What ALSO landed with it: `runner/transports/http.py` + `http_blobs.py` and the `http(s)://` address (ADR 0016); `data/stores/strangeloop.py` and the observer reading `strangeloop://` (ADR 0015); `runner/exporters/wandb.py`; HostService serving learner verbs off the loop with admission held to completion (an HTTP timeout does not end GPU work). The one Desk change is `automatic_recovery`, on by default and off in the Strange Loop configs until the live termination drill (ADR 0014, Q12).

    NOT published from the worktree: its retirement-custody rewrite of `runner/desk.py` (retiring metal, stopped instances, delivery-intent barriers, `RemoteHost.retire`) and the tests that ride it — ADR 0014 Part C lands that once, in the shared runtimes; `deploy/strangeloop_grpo.py` and the lamp/stream desk configs (inherited experiments); the worktree's inherited adapters, tasks, losses and study runners. Live Modal startup and Strange Loop image/SSH/storage/termination validation remain outstanding; no image, GPU or deployment was used to publish this. 1448 tests, 194 skipped.

92. **THE CHECKPOINT IS THE COMMIT, THE WIRE CARRIES THE POLICY, AND STOP IS A DISPOSITION (ADR 0014, 2026-09-13).** Samarth's ask, after two Codex threads spent a day fighting the harness: updates were slow because every one of them wrote every adapter and optimizer blob to the volume (a Q0 fit: 289 MB per update, median seal 31 s against 5–17 s of training on Modal; 22–108 s on Strange Loop); the `main` pool was mandatory even for a learner-only replay run; a stopped run had no word on the record and the recovery queue revived it; the idle rule released a pool under training runs anchored elsewhere; nothing in the desk could stop one run by hand. Implemented on `claude/adr-0014-commit-cadence` from `origin/main` 5d1abf1, with the Strange Loop backend published first (#91) so every part landed once in the shared runtimes. 1478 tests, 194 skipped, byte-identity kept at every cadence.
    - **A — the cadence is declared, the checkpoint is the durable point.** `Checkpointing(every, delivery)` (`runner/checkpointing.py`) rides the frame beside `subdir` and `resume` and is required at every door — `run_experiment`, `Host.submit`/`adopt`, `RemoteHost.adopt`, `Campaigns.submit`, `RemoteDesk.submit`, `migrate`, `VenueClient.submit`, both venue doors (`--every`, `--delivery`), every deploy entrypoint that submits — and defaulted nowhere (Q1). `checkpoints.jsonl` beside the ledger records `{update, versions}` after the blobs (Q3); checkpoint 0 is the initial blobs, adapters AND moments. Attach REWINDS to the checkpoint tail (Q2): ledger lines, waves, postdata above it and rollouts pinned past its versions are cut and regenerate. Retention reads the checkpoint record; `run_done` requires the extent sealed; a directory with no record is a pre-0014 run where every line was a checkpoint. `Checkpointing.due` is `u % every == 0`, the extent, and a drain (Q10: updates only). Measurement backfills checkpointed versions only (Q13).
    - **B — the wire, and there need not be one.** `serving_pools(spec)` is `main` on the policy base, or nothing: a learner alone opens no transport (the `main`-optional and `Learner.tokenize` halves of ADR 0011 adopted, #90). The Trainer's commit is three named steps — `deliver` (push to every serving pool over its transport, BEFORE the ledger line), `commit` (the line), `checkpoint` (blobs + record + sweep). `delivery="store"` is the non-default file-system path, bound to `every=1` (Q5). A miss on an uncheckpointed version is `BundleUnavailable`, parked and resumed from the checkpoint (Q4). The Modal mount hydrates the committed checkpoint record beside the ledger before a resume.
    - **C — stop is a disposition.** `Host.stop(drain=True)` raises the run's `StopRequest`; the Trainer reads it at its next wait or update boundary (a Trainer held waiting for a wave still drains), checkpoints its last commit, and `end_on_stop` ends every runner (Q6). The roster reads `running | done | stopped | failed`, with the error and whether the death was the wire's. The desk journals `stopped` (`Desk.stop`, `stop_subdir` — never revived by reap, registration or knock; a later delivered placement supersedes), `failed` (the experiment's own death, off the roster on the reaper's tick, never retried — Q7), `parked` (a wire death, retried). `RECOVERY_GENERATION` retired (Q8); `bootable_metals` stays. Doors: `deploy/desk.py::stop`/`::dispositions`, `deploy/strangeloop.py stop`/`dispositions`, `python -m rlstack fleet --desk <address> stop|dispositions|status`. The observer ranks stopped > failed > parked over a host journal's last word and shows `checkpointed` on every run row.
    - **D — idle by dependents, a clock per host, journals that batch, a watchdog that rechecks.** The idle rule's fourth yes: a listing a running placement routes through is busy (Q9; the 06:14 UTC outage). `Desk(host_idle_s=)`, the metal's limit unsaid, decarves an idle carved host before the metal's clock releases the container (Q9). Host journal lines stage and ride the next commit or a 30 s flush timer (Q11); the fleet journal stays per event. A stall verdict is confirmed by one more heartbeat before it costs a host, and the kill comes before the journal line.
    - **E — unification.** #91 published the Strange Loop worktree (ADRs 0015–0017); this entry's parts landed in `DeskRuntime`/`MetalRuntime` for both providers. NOT ported from that worktree, by decision: its desk custody rewrite (`retiring`, `stopped_instances`, `HostService.retire`) — Part C is its replacement; and `StrangeLoopLocalStore` publishing at `_persist` points rather than per mutation, which its per-write contract and four of its tests encode and which wants a note of its own before it moves.
    - **Q15/Q16.** The runner stays a task of its anchor host; the process split is the next ADR. The noun is RUNNER: `runner/daemons/` → `runner/roles/`, `Daemon` → `Runner`, `RunnerNeed`, `plan_runners`; hosts are the standing services and the three roles are their clients.
    - **Unproven, stated:** no live drill on Modal or Strange Loop yet (a spaced-checkpoint run killed mid-interval and resumed; the drain through a real `PipeTransport`; HTTP blob throughput over SSH); the seal on a checkpoint update itself is unchanged in size; the frozen-part emission (the 96 MB decoder at every version) is deferred to its own ADR (Q14).

95. **VERIFIED IMMUTABLE STORE READS, SHARED POD CACHE, NO AUTOMATIC REFRESH (ADR 0020, 2026-09-19).** Samarth approved the narrowed ADR and implementation: the store owns verification and caching; run configuration and shared runners own future prefetch and publication timing. CAS and named-adapter payloads use `_read_hashed` / `_write_hashed`. Every returned payload matches its expected sha256, including the named in-process LRU. Strange Loop confirms hashed publication with authoritative file size instead of full payload readback; metadata still seals a confirmed payload with full readback. This moves detection of same-size corruption to a reader and supplies no transaction or automatic repair guarantee. Mutable reads, writes and custody remain ADR 0015's.

    Strange Loop's bounded local `BlobCache` coordinates fetches across processes using file locks, installs complete bytes by rename, evicts old payloads and recovers orphan temporaries after a process dies. A verified already-visible mounted object can come from any pod; a miss or corrupt mounted copy falls through to HTTP without refreshing or modifying the mount. `MetalConfig` carries directory, byte bound and mount-read settings into the worker environment and spawned residents; example metals select `/tmp/rlstack-blobs`, 8 GiB and mount reads. No settings change run identity or stored run bytes. There is no prefetch daemon, batch-publication primitive, commit coalescing, checkpoint optimization or census rewrite in this change.

    **Validation:** 1,918 local tests, 255 environment-gated skips, no failures; resume tests and all fourteen canonical venue rows unchanged. 127 storage tests passed on a 2-CPU Strange Loop lease, including five live provider cases and independent-process locking/crash tests. The first live pass caught symlinked `/scratch` and `/persist` defeating cache-location validation; resolving the mount roots fixed it. `knowledge/evidence/0020-store-validation.json` links initial failures and final per-test measurements. No GPU throughput or provider failure-atomicity claim, and no production deployment. Isolated files and credential copies were removed and the test lease released.

96. **FREE-FORM DREAM FORKS AND MEASUREMENTS (research log 0009, 2026-09-20).** Samarth authorized the corrections, a matched oracle, inspection after two updates, then a full run if healthy. New menu tasks declare one document per dream, no decoys, fixed optimizer steps, the negative log-mean-exp fade score (tau and reference explicit), and invalid-dream handling. `fork_advantage` consumes validity and centers only valid dreams; invalid rows get the worst valid score or zero gradient. A group with no valid dream fails explicitly, and one valid dream has no comparison. Saved tasks missing these fields retain the old scoring behavior; the legacy stream advantage remains unchanged. No new learner verb or store layout. The fit client journals full before/after per-document probe vectors, stable row IDs, actual steps and phase timings, indexed by run and update. Optional loss components survive the learner wire and are retained per microbatch in committed ledgers; the existing objective reduction is unchanged. New preparation appends closed-book gold-answer probes as diagnostics and scores only rewrite rows. The oracle concatenates its topic text into one document under an explicit token cap and keeps its trained rewrite out of every scoring arm. Wall clocks remain in the host journal, outside deterministic run bytes. Validation: the full suite ran 1,930 tests successfully (256 environment-gated skips); 17 fork tests, 13 dream tests and eight end-to-end tests passed with torch; three local oracle/inspection tests passed. Live oracle and two-update gates are pending; offline success is not evidence of useful dreams. The shared W&B exporter also handles generation-only runs by reading each atomically sealed rollout once and exporting actual completion/token/truncation counts; it writes no ledger and supplies no invented training loss. The trained policy and store layout are unchanged.
...
- Your ENTIRE final message is the answer payload — no meta-commentary.
    ...
    - Inner fit traces now retain the first/last optimizer step of each stage and every tenth shared step in the existing host journal: row and job IDs, effective rates, timestamps, per-document NLL numerators/token counts, document denominators and combined loss. Standalone Fitter flushes at committed jobs; FitClient includes traces with fork diagnostics. No per-step network writes or timing fields in deterministic ledgers. Sequence-SFT gradients are unchanged; unequal-length document tests verify objective and gradients, including the factual-SFT KL term. Sixty fit-family tests, six torch loss tests and eight torch end-to-end tests pass. Older live oracle pods retain their original source and are not restarted just for added telemetry.
