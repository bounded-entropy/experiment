"""A SECOND METAL for the anger venue: the six arms beside the teacher's set.

    modal deploy deploy/anger_metal_b.py          # this metal's app
    modal run deploy/anger_metal_b.py::up         # boot it, wait for the desk
    modal run deploy/concept_anger.py::train --layer 10 --teacher-run <rid>
...
arm submitted through `concept_anger.py::train` lands here when the teacher's
...
The card, the idle clock and the recipe are `concept_anger.py`'s, restated
...
def proposed_recipe():
    """`concept_anger.proposed_recipe`, restated (see the module docstring)."""
