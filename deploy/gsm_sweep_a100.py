"""THE WITHIN-TASK SWEEP: one card, 10 train families x 4 methods = 40 runs.
...
NEAR_EVAL = 48                 # ALL held-out instances of the train family
...
RUNS_KEY = "measurements/gsm-sweep/runs.json"      # {run_id: {...}} — the cron's list
    ...
    def bring_up(self) -> None:
        ...
        class GsmDeskStore(ModalVolumeStore):
            ...
            def append_fleet_event(self, entry) -> None:
                self._append_line("fleet/gsm-sweep.jsonl", json.dumps(
                    entry, sort_keys=True, separators=(",", ":")))

            def read_fleet_log(self):
                try:
                    raw = self._read("fleet/gsm-sweep.jsonl").decode()
...
def bring_up_metal(name: str):
    ...
    service = MetalService(
        ...
        engine_factory=lambda regime, partition: VllmEngine(
            ...
            max_model_len=2048, max_bundles=64, max_rank=16,
...
def family_task_sets(store, rows, family: int):
    """(train uri, eval uri, train ids) for ONE family's within-task sweep:
    train on TRAIN_INSTANCES of `family`, evaluate on ALL its held-out
    instances and NOTHING else — the question is within-task transfer, so
    cross-family tasks stay out of the measurement."""
    ...
    chat = chat_formatter()
    train = gsm_train_tasks(rows, family, TRAIN_INSTANCES, chat)
    held = gsm_eval_tasks(rows, family, TRAIN_INSTANCES, (family,),
                          near=NEAR_EVAL, far=0, chat=chat)
...
METHODS = ("grpo", "spectral", "slatent", "sdpo")


def arm_tags(method: str, family: int):
    base = {
        "grpo": (["gsm-sweep", "grpo", "lora-baseline"],
                 "plain LoRA GRPO"),
        "spectral": (["gsm-sweep", "spectral", "svf"],
                     "SVF: trainable singular-value gains, top-k served"),
        "slatent": (["gsm-sweep", "spectral", "latent", "svf", "gated"],
                    "SVF gains generated from a latent, gated KL"),
        "sdpo": (["gsm-sweep", "sdpo", "reflect-loop"],
                 "iterative reflect loop, final-turn cloning"),
    }[method]
    return (base[0] + [f"train-f{family:02d}"],
            f"{base[1]} — within-task transfer on family {family}")


def campaign_specs(store) -> tuple[dict, dict]:
    """The WITHIN-TASK sweep as values: every chosen family takes a turn as
    ...
    rows_cache = gsm_rows(store)
    env = "math_single_turn"
    specs: dict = {}
    meta: dict = {}
    for family in verdict["chosen"]:
        train_uri, eval_uri, train_ids = family_task_sets(
            store, rows_cache, family)
        plans = rl_plans(store, env, train_ids)
        loop = loop_plans(store, env, train_ids)

        def arm(bank, loss, post, lr, overrides={}, plans=plans,
                envs=(env,), makers=(), lag=1, train_uri=train_uri,
                eval_uri=eval_uri):
        ...
        four = {
            "grpo": arm({"pi": lora(SITE, r=16)}, "grpo",
                        ("final_answer", "grpo_advantage"), 1e-4),
            "spectral": arm({"pi": spectral(SITE, k=SPECTRAL_K)}, "grpo",
            ...
            "sdpo": arm({"pi": lora(SITE, r=16)}, "sdpo",
                        ("final_answer",), 1e-4, plans=loop,
                        envs=(env, "reflect_retry"), makers=("reflect",),
                        lag=0),
        }
        for method, spec in four.items():
            name = f"{method}-f{family:02d}"
            specs[name] = spec
            meta[name] = {"family": family, "method": method,
                          "eval": eval_uri}
    return specs, meta
...
def build_campaign() -> dict:
    ...
    store = ModalVolumeStore("/store", volume=store_volume, locator=STORE)
    specs, meta = campaign_specs(store)
    rows = {name: json.loads(canonical_json(spec))
            for name, spec in specs.items()}
    store_volume.commit()
    return {"specs": rows, "meta": meta}
...
def write_roster(roster: dict) -> None:
    ...
    for rid, entry in roster.items():
        tags, note = arm_tags(entry["method"], entry["family"])
...
async def campaign() -> None:
    """The WITHIN-TASK sweep through the desk: 10 families x 4 methods = 40
    tenancies on one card; the roster lands at RUNS_KEY and every accepted
    run is tagged (method + train-f<NN>) in annotations.jsonl."""
    from rlstack.runner.remote import RemoteDesk, spec_from_json

    told = await build_campaign.remote.aio()
    rows, meta = told["specs"], told["meta"]
    fleet = RemoteDesk(DeskTransport())
    roster: dict = {}
    for name in sorted(rows):
        ...
        print(f"[{name}] accepted={reply.get('accepted')} "
              ...
              f"reply={json.dumps(reply, default=str)[:200]}")
        if reply.get("accepted"):
            roster[reply["run_id"]] = {"arm": name, **meta[name]}
