"""EngineSampleClient: the concrete SampleClient (rlstack/client.py protocol).

One instance per episode per pool. `pool(name)` returns a sibling for another
named engine pool — the whole inference side is reachable from any environment
or postprocessor — and all siblings share ONE seed sequence, so multi-pool
traffic is deterministic regardless of which pools an episode touches.
"""
...
from rlstack.data.trajectory import Message, Role, Turn
