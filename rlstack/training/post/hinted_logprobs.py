...
the answer — one model, no second pool. The hint is task metadata, read by the
one convention `data.trajectory.hint_for` states. Scoring is seedless, so
adding this processor never shifts the run's sampling seeds.
"""
