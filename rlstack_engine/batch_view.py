...
the constructor that reads engine internals (which token belongs to which
request, which request pins which bundle) is the ONLY code that tracks the
engine's metadata layout across versions. When vLLM moves a field, this file
changes and no plugin does.
"""
...
class BatchView:
    ...
    the multi-tenant gather index. `is_decode[i]` distinguishes decode rows
    (attend to every side position) from prefill rows (causal against them).
    """
    ...
    def from_vllm(attn_metadata: object, layer_idx: int,
        ...
        """Build the view from vLLM attention metadata — the B3 seam."""
